...
print("Top 5 by betweenness centrality (bridges/chokepoints in the grid):")
...
print("\nTop 5 by degree centrality (most directly connected):")
...
print("\nSaved full centrality table -> substation_centrality.csv")

print("\n--- VULNERABILITY ANALYSIS ---")

bridges = list(nx.bridges(SG))
print(f"Bridges (single lines whose removal disconnects the grid): {len(bridges)}")
for b in bridges:
    print(f"   {b[0]} -- {b[1]}")

cut_vertices = list(nx.articulation_points(SG))
print(f"\nArticulation points (substations whose failure splits the network): {len(cut_vertices)}")
print(cut_vertices)
...
nodes = nx.draw_networkx_nodes(
    SG, pos_geo, node_size=node_sizes, node_color=node_colors,
    cmap=plt.cm.viridis
)
...
nx.draw_networkx_nodes(
    SG, pos_spring, node_size=[100 + 3000 * degree_centrality[n] for n in SG.nodes()],
    node_color=node_colors_region, cmap=plt.cm.tab10
)
nx.draw_networkx_edges(SG, pos_spring, alpha=0.4, edge_color="gray")
nx.draw_networkx_labels(SG, pos_spring, font_size=6)
plt.title("Ghana National Grid Network (spring layout)\nNode size = degree centrality, color = region")
plt.axis("off")
plt.tight_layout()
plt.savefig("grid_network_spring.png", dpi=150)
plt.close()

print("\nSaved plots -> degree_distribution.png, grid_network_geographic.png, grid_network_spring.png")
...
if nx.has_path(SG, src, dst):
    ...
    print(f"No path currently exists between {src} and {dst}")

print("\nDone. Hand off substation_centrality.csv + PNGs to Aaron for the dashboard/report.")
